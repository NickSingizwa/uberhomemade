...
export const UberProvider = ({ children }) => {
  ...
  const checkIfWalletConnected = async () => {
    ...
    try{
      ...
      if(addressArray.length > 0){
        ...
        requestToCreateUserSanity(addressArray[0])
      }
    }catch(e){
      console.log('error',e)
    }
  ...
  const requestToCreateUserSanity = async (address) => {
    if(!window.ethereum) return
    try{
  ...
  useEffect(() => {
    if(!currentAccount) return
      ...
      requestCurrentUserInfo(currentAccount);
    }
  },[currentAccount])
  ...
  const requestCurrentUserInfo = async walletAddress => {
    try{
  ...
  //provider for acces to the whole app
  return (
